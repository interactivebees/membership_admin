...
class HospitalMemberRegistersController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Hospital Member Register';
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('form_type','hospital');

        $grid->column('id', __('Id'));
        $grid->column('user_id', __('User id'));
        $grid->column('hospital_type', __('Hospital type'));
        $grid->column('form_type', __('Form type'));
        $grid->column('parent_hospital_name', __('Parent hospital name'));
        $grid->column('no_of_hospital_in_parent', __('No of hospital in parent'));
        $grid->column('organization_name', __('Organization name'));
        $grid->column('hospital_category_id', __('Hospital category id'));
        $grid->column('membership_category_id', __('Membership category id'));
        $grid->column('hospital_email', __('Hospital email'));
        $grid->column('membership_number', __('Membership number'));
        $grid->column('numbers_of_beds_in_hospital', __('Numbers of beds in hospital'));
        $grid->column('hospital_address', __('Hospital address'));
        $grid->column('pan_no', __('Pan no'));
        $grid->column('gst_registration_number', __('Gst registration number'));
        $grid->column('hospital_mobile', __('Hospital mobile'));
        $grid->column('hospital_social', __('Hospital social'));
        $grid->column('hospital_logo', __('Hospital logo'));
        $grid->column('accreditation_category', __('Accreditation category'));
        $grid->column('tier_classification', __('Tier classification'));
        $grid->column('hospital_location', __('Hospital location'));
        $grid->column('core_services_hospital', __('Core services hospital'));
        $grid->column('support_services_hospital', __('Support services hospital'));
        $grid->column('diagnostic_centre', __('Diagnostic centre'));
        $grid->column('role', __('Role'));
        $grid->column('specialization', __('Specialization'));
        $grid->column('area_of_interest', __('Area of interest'));
        $grid->column('organization_head', __('Organization head'));
        $grid->column('contact_person_representing_organization_one', __('Contact person representing organization one'));
        $grid->column('contact_person_representing_organization_two', __('Contact person representing organization two'));
        ...
        $grid->column('personal_address', __('Personal address'));
        $grid->column('personal_social', __('Personal social'));
    ...
    protected function form()
    {
        ...
        $form->text('form_type', __('Form type'));
        $form->text('parent_hospital_name', __('Parent hospital name'));
        $form->number('no_of_hospital_in_parent', __('No of hospital in parent'));
        $form->text('organization_name', __('Organization name'));
        $form->text('hospital_category_id', __('Hospital category id'));
        $form->text('membership_category_id', __('Membership category id'));
        $form->text('hospital_email', __('Hospital email'));
        $form->text('membership_number', __('Membership number'));
        $form->text('numbers_of_beds_in_hospital', __('Numbers of beds in hospital'));
        $form->text('hospital_address', __('Hospital address'));
        $form->text('pan_no', __('Pan no'));
        $form->text('gst_registration_number', __('Gst registration number'));
        $form->text('hospital_mobile', __('Hospital mobile'));
        $form->text('hospital_social', __('Hospital social'));
        ...
        $form->text('organization_head', __('Organization head'));
        $form->text('contact_person_representing_organization_one', __('Contact person representing organization one'));
        $form->text('contact_person_representing_organization_two', __('Contact person representing organization two'));
        ...
        $form->text('personal_address', __('Personal address'));
        $form->text('personal_social', __('Personal social'));
